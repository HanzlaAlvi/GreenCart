...
try {
  ...
  if (result.success) {
    toast({ title: "Order Confirmed!", className: "bg-[#5E8B4F] text-white" });
    navigate("/shop/orders");
  }
} catch (error) {
  toast({
    title: "Order Failed",
    description: error?.response?.data?.message || error.message || "Order failed",
    variant: "destructive"
  });
} finally {
...
{/* Banner */}
<motion.div className="relative h-[300px] w-full overflow-hidden" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>


<motion.div className="absolute bottom-8 left-0 right-0 container mx-auto px-4" initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.3 }}>

<motion.div animate={{ rotate: [0, 10, -10, 0], scale: [1, 1.1, 1] }} transition={{ repeat: Infinity, repeatType: "reverse", duration: 3 }}>
...
Checkout
  ...
  {/* Main Content */}
  <motion.div className="container mx-auto px-4 py-8 flex-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      {/* Order Summary */}
      <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.6 }} className="bg-white rounded-xl shadow-lg border border-[#A7BC8A]/20 p-6">
        ...
        <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2">
          {cartItems?.items?.length > 0 ? (
            cartItems.items.map((item, index) => (
              <motion.div key={`${item.productId}-${index}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * index }}>
                <UserCartItemsContent cartItem={item} />
              </motion.div>
            ))
          ) : (
            <motion.div className="flex flex-col items-center justify-center py-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <div className="mt-6 pt-6 border-t border-[#A7BC8A]/20">
          ...
          <motion.div className="mt-8" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            <Button
